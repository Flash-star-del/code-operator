...
4. 可验证证据：真实缺陷修复任务（初始 2 failed，修复后 3 passed，6 轮 6 次工具调用）与冻结黄金任务三次全部通过的脱敏证据存于 docs/evidence；离线测试 870+ 项，测试进程主动拒绝真实网络连接。docs/agent-comparison-results.md 另记录与 Kimi Code 的双系统横评（五个合成任务、单次运行、独立隐藏测试评分）。
