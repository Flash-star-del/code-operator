...
一、安装与启动
要求 Python 3.11。运行依赖使用“python -m pip install -r requirements.txt”安装。程序完成 M1 实现并通过验证后，在此补充准确启动命令。运行时必须通过环境变量提供 CODE_OPERATOR_API_KEY、CODE_OPERATOR_BASE_URL 和 CODE_OPERATOR_MODEL；API Key 示例统一写为 <YOUR_API_KEY>。
