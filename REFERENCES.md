...
R0 表中的十个名称是当时冻结的计划场景；实现阶段按最终模块职责调整了部分函数名。2026-08-28 的 E1 复核确认四行来源均保留完整 URL、40 位 commit、许可证和阅读文件；只读 GitHub REST 检查也确认四个 commit 精确存在，对应 `LICENSE`/`LICENSE.md` 仍包含记录的 Apache-2.0、MIT 或保留权利标记。此次检查不打开新来源、不更新快照。每个采用项已有当前仓库中真实存在的测试，对应关系如下：

| R0 场景 | 当前可执行测试 |
|---|---|
| T01 缺失 ID | `test_missing_tool_envelope_fields_are_protocol_errors` |
| T02 重复 ID | `test_duplicate_tool_call_ids_are_protocol_errors`、`test_empty_or_duplicate_call_ids_are_protocol_errors` |
| T03 坏参数回灌 | `test_bad_arguments_still_produce_one_matching_tool_result`、`test_invalid_arguments_return_one_matched_failure` |
| T04 多调用含拒绝 | `test_multi_call_turn_returns_exactly_one_result_for_each_failure_kind` |
| T05 输出头尾截断 | `test_stdout_and_stderr_use_head_tail_truncation`、`test_command_timeout_and_output_truncation_are_replayed` |
| T06 完整回合裁剪 | `test_trim_removes_oldest_complete_round_without_splitting_tool_pair`、`test_agent_loop_crops_only_oldest_complete_tool_round` |
| T07 三次重复停止 | `test_repeated_call_stops_after_third_equal_result` |
| T08 Ctrl-C 清理 | `test_main_interpreter_interrupt_stops_active_process_promptly`、`test_user_aborted_tool_stops_loop_before_another_model_request` |
